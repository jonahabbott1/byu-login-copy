...
;(function (t, r) {
  ...
  if (Gt < -720) {
    ...
    zt = !/^Thu Dec 10 2015 \d\d:\d\d:\d\d GMT[-\+]\d\d\d\d(?: |$)/.test(
      Jt.toString()
    )
  } else {
    Zt = $t.toDateString() !== "Mon Jan 01 -45875"
    zt = !/^Wed Dec 09 2015 \d\d:\d\d:\d\d GMT[-\+]\d\d\d\d(?: |$)/.test(
      Jt.toString()
    )
  }
  ...
  var ar = "-000001"
  var or =
    Date.prototype.toISOString && new Date(ir).toISOString().indexOf(ar) === -1
  var fr =
  ...
  var lr = (function () {
    try {
      return (
        ...
        new Date(ir).toJSON().indexOf(ar) !== -1 &&
